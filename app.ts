...
import chalkAnimation, { neon } from "chalk-animation"; // Importing chalk-animation for animations
...
async function box() {
  let Animation_box = chalkAnimation.neon(
    boxen(`Quiz System\nProject 08\nBy\n*Anas Hakeem*`, {
  ...
  if (initialAnswer.action === "Start Quiz") {
    ...
    const quizQuestions = [
      ...
      },
      {
        type: "list",
        name: "mountain",
        message: chalk.bgGreen.white.italic.underline(
          `Which mountain range is located in the north of Pakistan ?`
        ),
      ...
      },
      {
        type: "list",
        name: "primeMinister",
        message: chalk.bgRed.white.italic.underline(
          `Who is the current Prime Minister of Pakistan (as of 2024) ?`
        ),
        choices: [
          "Imran Khan",
          "Nawaz Sharif",
    ...
    // Display result
    console.log(
      chalk.bgWhite.red.italic.underline(
        `\n\t\t\t\t\tHello ${answers.UserName}! You scored ${score} out of ${
